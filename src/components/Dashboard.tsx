...
import { createSharedAccount, findSharedAccountByUserId } from '../api/sharedAccountApi'
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    // Check if invitation link is already in localStorage
    const savedInvitationLink = localStorage.getItem('invitationLink');
    if (savedInvitationLink) {
      setInvitationLink(savedInvitationLink);
    } else if (session && session.user) {
      // If no saved link, fetch shared account data
      findSharedAccountByUserId(session.user.id)
        .then((data) => {
          const link = `http://localhost:5173/invite?code=${data.unique_code}`
          setInvitationLink(link);
          localStorage.setItem('invitationLink', link);
        })
        .catch((error) => console.error('Error fetching shared account:', error.message))
    }
  }, [session])
  ...
  const openModal = async () => {
    ...
    if (!invitationLink) {
      ...
      localStorage.setItem('invitationLink', newInvitationLink);
